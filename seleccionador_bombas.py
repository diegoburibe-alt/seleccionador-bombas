...
class PumpCurveBase:
    def __init__(self, diam: float, puntos: List[Dict[str, float]]) -> None:
        ...
        p_raw = np.array([p.get("pot_kw", np.nan) for p in self.puntos], dtype=float)
        ...
        self.power_q = unique_q
        self.power_values = p_raw[unique_idx]
        valid_power = ~np.isnan(self.power_values)
        self.has_power_data = bool(np.sum(valid_power) >= 2)
        if self.has_power_data:
            self.power_q = self.power_q[valid_power]
            self.power_values = self.power_values[valid_power]
            self.interp_power = PchipInterpolator(self.power_q, self.power_values)
        else:
            self.interp_power = None
    ...
    def get_power(self, q: float, density: float = 1000.0, viscosity_cf: float = 1.0) -> float:
        if self.has_power_data and self.interp_power is not None:
            q_eval = float(np.clip(q, self.power_q[0], self.power_q[-1]))
            p_ref = float(self.interp_power(q_eval))
            return p_ref * (density / 1000.0) / max(viscosity_cf, 0.01)
        ...
        for name, group in df.groupby(groupby_cols, dropna=False):
            ...
            for d in diametros:
                ...
                for _, row in group_d.iterrows():
                    ...
                    pot_kw = safe_float(row.get("pot_kw", row.get("Pot_kW", row.get("P_kW", np.nan))), np.nan)
                    if not np.isnan(q) and not np.isnan(h):
                        puntos.append({"Q": q, "H": h, "eta": eta, "NPSH": npsh, "pot_kw": pot_kw})
...
# Cálculo hidráulico
# ==============================
def find_trim_diameter(q_req: float, h_req: float, base_curve: PumpCurveBase, d_min: float, d_max: float) -> Optional[float]:
...
def find_operating_point(trim_curve: TrimmedCurve, sys_curve: Optional[SystemCurve], q_max: Optional[float] = None) -> Optional[float]:
...
def render_top_header() -> None:
    col_logo, col_title = st.columns([1.1, 5], vertical_alignment="center")
    with col_logo:
        ...
        rows.append(
            {
                ...
                "D mín. (mm)": fam["D_min"],
                "D máx. (mm)": fam["D_max"],
                "Diámetros disponibles": ", ".join(str(int(d)) if float(d).is_integer() else f"{d:.1f}" for d in fam["diametros_disponibles"]),
                "_fam": fam,
            }
        )
...
def evaluate_families(
    ...
    for fam in families:
        ...
        op_q = find_operating_point(trim_curve, sys_curve, q_max=base_curve.q_max * trim_curve.ratio)
        if op_q is None:
            op_q = q_req
...
def raw_curve_points(
    curve_obj: PumpCurveBase,
    metric: str,
    density: float = 1000.0,
    visco_cf: float = 1.0,
) -> Tuple[np.ndarray, np.ndarray]:
    df = pd.DataFrame(curve_obj.puntos).sort_values("Q")
    df = df.groupby("Q", as_index=False).mean(numeric_only=True)

    q_vals = df["Q"].to_numpy(dtype=float)
    if metric == "H":
        y_vals = df["H"].to_numpy(dtype=float)
    elif metric == "ETA":
        y_vals = df["eta"].to_numpy(dtype=float) * visco_cf
    elif metric == "NPSH":
        y_vals = df["NPSH"].to_numpy(dtype=float)
    elif metric == "P":
        if "pot_kw" in df.columns and not df["pot_kw"].isna().all():
            y_vals = df["pot_kw"].to_numpy(dtype=float) * (density / 1000.0) / max(visco_cf, 0.01)
        else:
            eta_vals = np.maximum(df["eta"].to_numpy(dtype=float) * visco_cf, 0.01)
            h_vals = df["H"].to_numpy(dtype=float)
            y_vals = ((q_vals / 3600.0) * h_vals * density * 9.81 / (eta_vals / 100.0)) / 1000.0
    else:
        y_vals = np.zeros_like(q_vals)

    return q_vals, y_vals


def plot_family_metric_manual(
    fam: Dict,
    metric: str,
    title: str,
    ...
    if show_all_diameters:
        for curve in fam["curvas"]:
            qq, yy = raw_curve_points(curve, metric, density=density, visco_cf=visco_cf)
            fig.add_trace(
                go.Scatter(
                    x=qq,
                    y=yy,
        ...
        exact_curve = None
        if selected_real_diam is not None:
            for curve in fam["curvas"]:
                if abs(curve.diam - selected_real_diam) < 1e-6:
                    exact_curve = curve
                    break

        if exact_curve is not None:
            qq, yy = raw_curve_points(exact_curve, metric, density=density, visco_cf=visco_cf)
            fig.add_trace(
                go.Scatter(
                    x=qq,
                    y=yy,
                    mode="lines",
                    name=f"D={exact_curve.diam:.0f} mm",
                    line=dict(width=3, color="black", shape="spline"),
                    hovertemplate="Q: %{x:.2f} m³/h<br>Valor: %{y:.2f}<extra></extra>",
                )
            )
        elif selected_curve_obj is not None:
            if hasattr(selected_curve_obj, "q_min") and hasattr(selected_curve_obj, "q_max"):
                q_min_plot = selected_curve_obj.q_min
                q_max_plot = selected_curve_obj.q_max
            else:
                q_min_plot = max(0.05, selected_curve_obj.base.q_min * selected_curve_obj.ratio)
                q_max_plot = selected_curve_obj.base.q_max * selected_curve_obj.ratio
            qq = np.linspace(q_min_plot, q_max_plot, 220)
            yy = curve_values(selected_curve_obj, metric, qq, density=density, visco_cf=visco_cf)
            fig.add_trace(
                go.Scatter(
                    x=qq,
                    y=yy,
    ...
    fig.update_xaxes(title_text="Caudal Q (m³/h)", showgrid=True, gridcolor="rgba(200,200,200,0.35)", zeroline=False)
    fig.update_yaxes(title_text=metric_label(metric), showgrid=True, gridcolor="rgba(200,200,200,0.35)", zeroline=False)
    return fig


def render_characteristic_curves_manual(
    fam: Dict,
    title_prefix: str,
    show_all_diameters: bool,
    selected_curve_obj=None,
...
def plot_family_metric(
    ...
    if selected_trim is not None:
        ...
        qq_sel = np.linspace(max(0.05, selected_trim.base.q_min * selected_trim.ratio), q_max_plot, 180)
        fig.add_trace(
            go.Scatter(
                x=qq_sel,
                y=curve_values(selected_trim, metric, qq_sel, density=density, visco_cf=visco_cf),
                mode="lines",
                name=(f"Diámetro escogido = {selected_trim.diam:.1f} mm" if selected_real_diam is None else f"D = {selected_real_diam:.1f} mm"),
                line=dict(width=3, color="#0059aa"),
                hovertemplate="Q: %{x:.2f} m³/h<br>Valor: %{y:.2f}<extra></extra>",
            )
        )
    elif not show_all_diameters and fam["curvas"]:
        curve = min(fam["curvas"], key=lambda c: abs(c.diam - (selected_real_diam or fam["curvas"][-1].diam)))
        qq = np.linspace(curve.q_min, curve.q_max, 140)
        fig.add_trace(
            go.Scatter(
                x=qq,
...
def hydraulic_selection_view(families: List[Dict]) -> None:
    ...
    df_res = pd.DataFrame(results).drop(columns=["_trim", "_fam", "_sys_curve", "_q_req", "_h_req", "_visco_cf", "_densidad"])
    ordered_cols = [
        "Serie (Marca)", "Marca", "Modelo", "Polos", "RPM", "D_Impulsor (mm)",
        "Q Op. (m3/h)", "H Op. (m)", "Eficiencia (%)", "Potencia (kW)", "NPSHr (m)", "Status NPSH",
    ]
    df_res = df_res[ordered_cols]
    st.dataframe(df_res.style.map(npsh_style, subset=["Status NPSH"]), use_container_width=True, hide_index=True)
    ...
    selected_idx = st.selectbox("Selecciona una bomba para revisar curvas", range(len(results)), format_func=lambda i: labels[i])
    ...
    st.dataframe(family_curve_summary_table(fam), use_container_width=True, hide_index=True)
...
def manual_selection_view(families: List[Dict]) -> None:
    ...
    selection_index = st.selectbox("Selecciona una bomba", range(len(filtered)), format_func=lambda i: selection_labels[i])
    selected_row = filtered.iloc[selection_index]
    fam = selected_row["_fam"]

    selected_real_diam = None
    if not show_all_diameters:
        selected_real_diam = st.selectbox("Diámetro a mostrar", fam["diametros_disponibles"], format_func=lambda x: f"{x:.1f} mm")
    ...
    selected_curve_obj = None
    if not show_all_diameters and selected_real_diam is not None:
        exact_curve = next((c for c in fam["curvas"] if abs(c.diam - selected_real_diam) < 1e-6), None)
        if exact_curve is None:
            selected_curve_obj = resolve_manual_curve(fam, float(selected_real_diam))
        else:
            selected_curve_obj = exact_curve

    render_characteristic_curves_manual(
        fam=fam,
