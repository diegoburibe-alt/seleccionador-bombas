...
NPSH_MARGIN_M = 0.5WATER_DENSITY_KG_M3 = 998.0VALID_USERNAME = st.secrets["auth"]["username"]VALID_PASSWORD = st.secrets["auth"]["password"]APP_SUBTITLE = "Series N-NP-N(V)"APP_TITLE = "Seleccionador Bombas Normalizadas"

IEC_MOTORS_KW = [0.18, 0.25, 0.37, 0.55, 0.75, 1.1, 1.5, 2.2, 3.0, 4.0, 5.5, 7.5,11, 15, 18.5, 22, 30, 37, 45, 55, 75, 90, 110, 132, 160, 200, 250,315, 355, 400, 500, 630,]
...
def file_to_base64(path: str) -> str:with open(path, "rb") as f:return base64.b64encode(f.read()).decode("utf-8")

def svg_to_data_uri(path: str) -> str:return f"data/svg+xml;base64,{file_to_base64(path)}"

def png_to_data_uri(path: str) -> str:ext = os.path.splitext(path)[1].lower().replace(".", "")if ext == "jpg":ext = "jpeg"return f"data/{ext};base64,{file_to_base64(path)}"

def image_to_data_uri(path: str) -> str:if path.lower().endswith(".svg"):return svg_to_data_uri(path)return png_to_data_uri(path)

def find_logo_path() -> Optional[str]:possible_paths = ["Logo Vogt.svg","Logo_Vogt.svg","logo_vogt.svg","Logo Vogt.png","logo_vogt.png",os.path.join(os.getcwd(), "Logo Vogt.svg"),os.path.join(os.getcwd(), "Logo_Vogt.svg"),os.path.join(os.getcwd(), "logo_vogt.svg"),os.path.join(os.getcwd(), "Logo Vogt.png"),os.path.join(os.getcwd(), "logo_vogt.png"),"/mnt/data/Logo Vogt.svg","/mnt/data/Logo_Vogt.svg","/mnt/data/logo_vogt.svg","/mnt/data/Logo Vogt.png","/mnt/data/logo_vogt.png",]for path in possible_paths:if os.path.exists(path):return pathreturn None

def inject_css() -> None:st.markdown(""".main-block {padding-top: 0.35rem;}
...
def safe_float(value, default=np.nan) -> float:try:return float(str(value).replace(",", ".").strip())except Exception:return default

def fmt2(value) -> str:if value is None or (isinstance(value, float) and pd.isna(value)):return "-"return f"{float(value):.2f}"

def fmt1(value) -> str:if value is None or (isinstance(value, float) and pd.isna(value)):return "-"return f"{float(value):.1f}"

def fmt0(value) -> str:if value is None or (isinstance(value, float) and pd.isna(value)):return "-"return f"{float(value):.0f}"

def style_numeric_df(df: pd.DataFrame,zero_dec_cols: Optional[List[str]] = None,one_dec_cols: Optional[List[str]] = None,):zero_dec_cols = set(zero_dec_cols or [])one_dec_cols = set(one_dec_cols or [])formatters = {}
...
def style_results_df(df: pd.DataFrame):styler = style_numeric_df(df,zero_dec_cols=["RPM", "Polos", "D_Impulsor (mm)"],one_dec_cols=["Q Op. (m3/h)", "H Op. (m)"],)if "Status NPSH" in df.columns:styler = styler.applymap(lambda x: "background-color: #fde2e1" if x is False else "",subset=["Status NPSH"])return styler

def keyify(prefix: str, value) -> str:return re.sub(r"[^a-zA-Z0-9_]+", "", f"{prefix}{value}")

def display_serie_value(marca: str, serie: Optional[str] = None) -> str:marca_txt = str(marca).strip().lower()if "grundfos" in marca_txt:return "N"if "sempa" in marca_txt:return "NP"if "wilo" in marca_txt:return "N (V)"if serie is not None and str(serie).strip() not in ["", "-"] and not pd.isna(serie):return str(serie)return str(marca)
...
def get_service_factor(power_kw: float) -> float:if power_kw < 22:return 1.15if power_kw < 55:return 1.10return 1.05

def select_motor(max_power_kw: float) -> float:req_power = max_power_kw * get_service_factor(max_power_kw)for motor_kw in IEC_MOTORS_KW:if motor_kw >= req_power:return motor_kwreturn IEC_MOTORS_KW[-1]

def poly2(x: np.ndarray, a: float, b: float, c: float) -> np.ndarray:return a * (x ** 2) + b * x + c

def infer_poles_from_rpm(rpm: Optional[float]) -> Optional[int]:if rpm is None or pd.isna(rpm):return Nonerpm = float(rpm)if rpm >= 2500:return 2if rpm >= 1200:return 4return 6

def discharge_from_model(modelo: str) -> Optional[int]:if not modelo:return Nonem = re.match(r"\s*(\d+)\s*-", str(modelo))return int(m.group(1)) if m else None

class PumpCurveBase:def init(self, diam: float, puntos: List[Dict[str, float]], q_max_override: Optional[float] = None) -> None:self.diam = float(diam)self.puntos = sorted(puntos, key=lambda p: p["Q"])
...
class TrimmedCurve:def init(self, base_curve: PumpCurveBase, trim_diam: float) -> None:self.base = base_curveself.diam = float(trim_diam)self.ratio = self.diam / self.base.diamself.q_min = self.base.q_min * self.ratioself.q_max = self.base.q_max * self.ratio
...
class InterpolatedDiameterCurve:def init(self,curve_low: PumpCurveBase,curve_high: PumpCurveBase,interp_diam: float,q_max_est: Optional[float] = None,) -> None:if curve_high.diam <= curve_low.diam:raise ValueError("Los diámetros vecinos deben estar ordenados de menor a mayor.")self.curve_low = curve_lowself.curve_high = curve_highself.diam = float(interp_diam)self.d1 = float(curve_low.diam)self.d2 = float(curve_high.diam)self.lam = (self.diam - self.d1) / (self.d2 - self.d1)self.q_min = max(curve_low.q_min, curve_high.q_min)
...
class SystemCurve:def init(self, h_stat: float, k: float) -> None:self.h_stat = float(h_stat)self.k = float(k)

def get_h(self, q: float) -> float:
    return self.h_stat + self.k * (q ** 2)

class PumpDatabase:def init(self) -> None:self.families: List[Dict] = []
...
def get_curve_q_max(curve_obj) -> Optional[float]:if curve_obj is None:return Noneif hasattr(curve_obj, "q_max"):q_max = safe_float(getattr(curve_obj, "q_max"), np.nan)return None if np.isnan(q_max) else float(q_max)if hasattr(curve_obj, "base") and hasattr(curve_obj, "ratio"):q_max = safe_float(curve_obj.base.q_max, np.nan) * safe_float(curve_obj.ratio, np.nan)return None if np.isnan(q_max) else float(q_max)return None

def blend_q_limit(q_low: Optional[float], q_high: Optional[float], lam: float) -> Optional[float]:if q_low is None and q_high is None:return Noneif q_low is None:return float(q_high)if q_high is None:return float(q_low)return float((1.0 - lam) * q_low + lam * q_high)

def qmax_por_afinidad_desde_diametro_maximo(fam: Dict, d_req: float) -> Optional[float]:curvas = sorted(fam["curvas"], key=lambda c: c.diam)if not curvas:return None
...
def find_trim_diameter(q_req: float,h_req: float,base_curve: PumpCurveBase,d_min: float,d_max: float,) -> Optional[float]:if d_min > d_max or d_min <= 0:return None
...
def find_interpolated_diameter_curve(fam: Dict,q_req: float,h_req: float,) -> Tuple[Optional[float], Optional[object], Optional[PumpCurveBase], Optional[PumpCurveBase]]:curvas = sorted(fam["curvas"], key=lambda c: c.diam)
...
def find_operating_point(trim_curve: TrimmedCurve,sys_curve: Optional[SystemCurve],q_max: Optional[float] = None,) -> Optional[float]:if sys_curve is None:return None
...
def viscosity_correction(viscosidad_cst: float) -> float:if viscosidad_cst <= 1.0:return 1.0if viscosidad_cst <= 5.0:return 0.97if viscosidad_cst <= 20.0:return 0.93return 0.88
...
def init_session_state() -> None:defaults = {"authenticated": False,"username": "","page": "login","loaded_families": None,"manual_selected_table_row": 0,"manual_selected_point_q": None,"manual_selected_point_curve_diam": None,"manual_selected_point_model": None,"manual_selected_point_show_all": None,}for key, value in defaults.items():if key not in st.session_state:st.session_state[key] = value
...
def render_top_header() -> None:logo_path = find_logo_path()
...
def go_to(page_name: str) -> None:st.session_state.page = page_namest.rerun()

def logout() -> None:st.session_state.authenticated = Falsest.session_state.username = ""st.session_state.page = "login"st.rerun()
...
def login_view() -> None:render_top_header()st.markdown("", unsafe_allow_html=True)
...
def main_menu_view() -> None:render_top_header()
...
def load_database_widget() -> Optional[List[Dict]]:uploaded_file = st.sidebar.file_uploader("Cargar Base Oficial (CSV)", type=["csv"])
...
def build_catalog_df(families: List[Dict]) -> pd.DataFrame:rows = []for fam in families:rows.append({"Serie": fam["serie_display"],"Modelo": fam["modelo"],"Polos": float(fam["polos"]) if fam["polos"] is not None else np.nan,"RPM": float(fam["rpm"]) if fam["rpm"] is not None else np.nan,"Descarga DN": float(fam["descarga_dn"]) if fam["descarga_dn"] is not None else np.nan,"D mín. (mm)": float(fam["D_min"]),"D máx. (mm)": float(fam["D_max"]),"Diámetros disponibles": ", ".join(str(int(d)) if float(d).is_integer() else f"{d:.2f}"for d in fam["diametros_disponibles"]),"_fam": fam,})return pd.DataFrame(rows)
...
def metric_label(metric: str) -> str:return {"H": "Altura H (m)","ETA": "Eficiencia (%)","P": "Potencia (kW)","NPSH": "NPSHr (m)",}[metric]

def curve_values(curve_obj,metric: str,q_values: np.ndarray,density: float = WATER_DENSITY_KG_M3,visco_cf: float = 1.0,) -> List[float]:values: List[float] = []for q in q_values:if metric == "H":values.append(curve_obj.get_h(float(q)))elif metric == "ETA":eta = curve_obj.get_eta(float(q)) if hasattr(curve_obj, "get_eta") else 0.0values.append(float(eta * visco_cf))elif metric == "P":values.append(float(curve_obj.get_power(float(q), density=density, viscosity_cf=visco_cf)))elif metric == "NPSH":values.append(float(curve_obj.get_npshr(float(q))))return values

def apply_graph_box(fig: go.Figure) -> None:fig.update_layout(plot_bgcolor="white",paper_bgcolor="white",)fig.update_xaxes(showline=True,linewidth=1.2,linecolor="black",mirror=True,ticks="outside",)fig.update_yaxes(showline=True,linewidth=1.2,linecolor="black",mirror=True,ticks="outside",)

def resolve_manual_curve(fam: Dict, requested_diam: float):for curve in fam["curvas"]:if abs(curve.diam - requested_diam) < 1e-9:return curve
...
def plot_family_metric(fam: Dict,metric: str,title: str,show_all_diameters: bool,selected_curve_obj=None,selected_real_diam: Optional[float] = None,op_q: Optional[float] = None,q_req: Optional[float] = None,h_req: Optional[float] = None,sys_curve: Optional[SystemCurve] = None,density: float = WATER_DENSITY_KG_M3,visco_cf: float = 1.0,black_curves: bool = False,smooth_curves: bool = False,highlight_curve_obj=None,highlight_q: Optional[float] = None,) -> go.Figure:fig = go.Figure()
...
def render_characteristic_curves_point(fam: Dict,show_all_diameters: bool,selected_curve_obj=None,selected_real_diam: Optional[float] = None,op_q: Optional[float] = None,q_req: Optional[float] = None,h_req: Optional[float] = None,sys_curve: Optional[SystemCurve] = None,density: float = WATER_DENSITY_KG_M3,visco_cf: float = 1.0,values_at_point: Optional[Dict[str, float]] = None,) -> None:st.subheader("Curvas Caracteristicas")c1, c2 = st.columns(2)c3, c4 = st.columns(2)
...
def render_manual_interactive_curves(fam: Dict,show_all_diameters: bool,selected_curve_obj=None,selected_real_diam: Optional[float] = None,density: float = WATER_DENSITY_KG_M3,visco_cf: float = 1.0,model_key: str = "",) -> None:st.subheader("Curvas Características")c1, c2 = st.columns(2)c3, c4 = st.columns(2)
...
def family_curve_summary_table(fam: Dict) -> pd.DataFrame:rows = []for curve in fam["curvas"]:rows.append({"Diámetro (mm)": round(curve.diam),"Q mín. (m3/h)": round(curve.q_min, 2),"Q máx. (m3/h)": round(curve.q_max, 2),"H @ Q mín. (m)": round(curve.get_h(curve.q_min), 2),"H @ Q máx. (m)": round(curve.get_h(curve.q_max), 2),"Q BEP (m3/h)": round(curve.q_bep, 2),"η BEP (%)": round(curve.eta_bep, 2),})return pd.DataFrame(rows)
...
def evaluate_families(families: List[Dict],q_req: float,h_req: float,npsha: float,densidad: float,viscosidad: float,usar_sistema: bool,h_est: float,k_sys: float,) -> List[Dict]:sys_curve = SystemCurve(h_stat=h_est, k=k_sys) if usar_sistema else Nonevisco_cf = viscosity_correction(viscosidad)results: List[Dict] = []
...
def hydraulic_selection_view(families: List[Dict]) -> None:st.sidebar.header("1. Parámetros del Fluido")fluid_name = st.sidebar.text_input("Fluido", "Agua limpia")densidad = st.sidebar.number_input("Densidad (kg/m³)", value=WATER_DENSITY_KG_M3, step=10.0, format="%.2f")viscosidad = st.sidebar.number_input("Viscosidad cinemática (cSt)", value=1.0, step=0.5, format="%.2f")
...
def reset_manual_filters():keys_to_delete = [k for k in st.session_state.keys() if k.startswith("manualflt_")]for key in keys_to_delete:del st.session_state[key]

def render_checkbox_filter_group(options: List,prefix: str,limit: int = 5,formatter=lambda x: str(x),):show_key = f"manualflt_showall_{prefix}"if show_key not in st.session_state:st.session_state[show_key] = False
...
def manual_selection_view(families: List[Dict]) -> None:st.markdown("### Seleccion de Bombas Manual")catalog_df = build_catalog_df(families)
...
def render_work_page_header() -> None:render_top_header()
...
def app() -> None:if not st.session_state.authenticated:login_view()return
